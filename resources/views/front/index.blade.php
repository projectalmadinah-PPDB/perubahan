...
<main class="w-full pt-14">
    ...
    <section 
        ...
        <h1 
            class="text-3xl md:text-5xl font-bold mb-0 leading-none title">
    ...
    {{-- alur --}}
    <section id="alur" class="w-full py-32 select-none bg-dasar">
        <div class="mx-auto container">
            <div class="w-11/12 lg:w-1/2 mx-auto">
                <div class="flex items-center justify-between bg-gray-200 h-1.5 relative">
                    ...
                    <div class="relative">
                        <div class="rounded-full w-6 h-6 bg-dasar shadow-md text-center border border-sekunder">
                            {{-- <i class="bi bi-check-lg text-white"></i> --}}
                        </div>
                        <p class="absolute text-center left-1/2 -translate-x-1/2 top-10 text-sekunder w-32 rounded-md bg-dasar shadow-lg text-xs tracking-wide leading-4 py-1 p-2">
    ...
    <!-- informasi -->
    <section 
        class="bg-sky-900 text-dasar py-16 px-20 flex flex-col items-center justify-center gap-y-2">
        ...
        <div class="swiper w-full">
            <div class="swiper-wrapper mx-auto w-full justify-start">
                @foreach ($article as $item)
                    <!-- foreach here -->
                <a href="{{ route('user.informasi.detail', $item->slug) }}" class="swiper-slide md:w-52 min-h-min max-h-48 bg-sky-900 border border-dasar rounded-xl py-5 px-8">
                    <article>
                        {{-- <img src="{{asset('/storage/'.$item['image'])}}" alt=""> --}}
                        <h1 class="text-xl mb-2 font-bold text-gray-100 tracking-wide truncate">{{$item->title}}</h1>
                        <p class="line-clamp-3 leading-relaxed tracking-wide text-gray-200">
                            {{$item->desc}}
                        </p>
                        <div class="flex justify-start items-center gap-x-2 mt-3">
                            <span class="italic ring-1 ring-sekunder bg-transparent rounded-full py-1 px-2 text-xs font-medium lowercase">{{ $item->category->name }}</span>
                            <span class="ring-1 ring-sekunder bg-transparent py-1 px-3 text-xs font-semibold">
